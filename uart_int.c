...
//{
//NVIC_SetPriority(UART0_IRQn,0);     //FOR SETTING PRIORITY TO THE INTERRUPT  
																		//FOR MULTIPLE INTERRUPT THE SET THE PRIORITY
	
//}

void UART0_IRQHandler(void)
{
	if(LPC_UART0->IIR & (1<<1))
	{
		LPC_GPIO2->FIODIR = (1<<9);
		LPC_GPIO2->FIOCLR = (1<<9);
	}
	if(LPC_UART0->IIR & (1<<2))
	{
...
void uart_init(void)
{
	...
	LPC_UART0->IER = (1<<1)|(1<<0);    // THRE ENABLE  INTERRUPT  AND SET RECEIVE ENABLE INTERRUPT
}
...
NVIC_EnableIRQ(UART0_IRQn);
	...
	while(ptr[i] != '\0')
	{
		...
		unsigned char data = uart_rx_data();
			uart_tx_data(data);
